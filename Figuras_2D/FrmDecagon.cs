...
namespace Figuras_2D
{
    public partial class FrmDecagon : Form
    {
        ...
        DecagonHelper op = new DecagonHelper();
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            if (float.TryParse(textBox1.Text, out float radioIngresado))
            {
                if (radioIngresado > 0)
                {
                    // ESCALADO AUTOMÁTICO (CLAVE)
                    float maxRadio = Math.Min(panel1.Width, panel1.Height) / 2 - 10;
                    radioVal = Math.Min(radioIngresado * 10, maxRadio);
                ...
                {
                    MessageBox.Show("Ingrese un radio válido mayor a 0.");
                }
            }
            else
            {
        ...
        private void panel1_Paint(object sender, PaintEventArgs e)
        {
            ...
            using (SolidBrush relleno = new SolidBrush(Color.Yellow))
            {
