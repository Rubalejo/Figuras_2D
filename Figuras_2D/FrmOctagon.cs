...
namespace Figuras_2D
{
    public partial class FrmOctagon : Form
    {
        ...
        // Clase helper (nombre único)
        OctagonHelper op = new OctagonHelper();
        ...
        private void panel1_Paint(object sender, PaintEventArgs e)
        {
            ...
            using (SolidBrush relleno = new SolidBrush(Color.Brown))
            {
    ...
    //  CLASE AUXILIAR SIN CONFLICTOS
    public class OctagonHelper
    {
        public PointF[] CalcularPuntos(int lados, float radio, int ancho, int alto)
        {
            ...
            {
                //  AJUSTE CLAVE PARA OCTÁGONO PERFECTO
                double angulo = (2 * Math.PI * i / lados) - (Math.PI / 8);
